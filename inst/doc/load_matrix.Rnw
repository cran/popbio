...
ceco<-matrix(ceco, nrow=3, byrow=TRUE, 
...
reshaped into a matrix.  In addition, stages (in position 1,
...
sapu<-matrix( as.numeric(grep("^[0-9.]+$", x1, value = TRUE)), nrow=4,byrow=TRUE, 
 dimnames=list(stages,stages))
sapu
...
elements since this technique is so useful in many other situations (e.g.,
using an expression of vital rates to calculate vital rate
sensitivities.  See the final example in this guide).

In this final \Rcmd{scan} example, first copy the pod-specific matrix elements for
...
Third, enter the  projection matrix from Fig 1 in Brault and Caswell (1993) into an expression.
The symbols in this matrix must match the column names in the matrix
of pod-specific rates above.  An alternative is to copy the matrix using \Rcmd{scan} and
then \Rcmd{parse} the output into an expression. 
...
Fourth,  apply the \Rcmd{eval} function to the \R~expression using a list of
matrix elements to get a vector of all 16 matrix elements from a single pod.
 For example, convert the matrix elements in the first row
to a list using \Rcmd{as.list} and then use \Rcmd{sapply} to evaluate
the expression in \Rcmd{podA} across each listed element.  


<<J01>>=
J01 <- sapply(podA, eval, as.list(pods[1,]))
J01
@ 
...
stages<-c("yearling", "juvenile", "mature", "postreprod")
 matrix(J01, nrow = 4, dimnames = list(stages, stages), byrow=TRUE)
@


Finally, you can combine the previous two steps in a loop and create
...
whales <- vector("list", 18)
 ...
 for (i in 1:18) {
  whales[[i]] <- matrix(sapply(podA, eval, as.list(pods[i,])), nrow = 4, byrow=TRUE, dimnames = list(stages, stages))
}
...
more projection matrices in each file. The formats of the files will
...
FE99<-matrix(x[1,], nrow=8, byrow=TRUE, dimnames=list(stages,stages))
image2(FE99, cex=.5)
...
arel<-split(x,1:13)
arel<-lapply(arel, matrix, nrow=8,  byrow=TRUE, dimnames=list(stages,stages))
...
markup) are used to construct matrix models of six  \emph{Orchis
...
expression (because of the Greek symbols,  I manually formatted
this expression instead of copying and pasting).  Finally, 
apply the \Rcmd{eval} function to the \R~expression using a list of
vital rates from a single site (arranged in rows) to
return a single matrix.  You can also create a list of all 30 matrices
following the code in the earlier killer whale example or even
calculate vital rate sensitivities and elasticities using the
\Rcmd{vitalsens} function in \Rcmd{popbio}. 

<<orchid>>=

## Orchis purpurea  eq. 1
orpu<-expression(
...
stages<-c("pcorm", "tuber", "sdlng", "juv", "nonfl", "flwer")

s1<-matrix( sapply( orpu, eval, as.list(vr[1,]) ), nrow=6, byrow=TRUE, dimnames=list(stages,stages))
s1
lambda(s1)
@
